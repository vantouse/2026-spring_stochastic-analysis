\graphicspath{ {./lab3/images/part2} }

\newpage
\section{Лабораторная работа № 3, часть 2: Исследование разброса случайных состояний системы вблизи устойчивого равновесия}
\subsection{Статистический анализ результатов прямого численного моделирования}
...
В модели FHN при значении параметра $a > 1$ равновесие $(\overline{x}, \overline{y}) = (-a, \frac{a^3}{3} - a)$ устойчиво. На основе симуляции длительностью $T = 100$ с шагом по времени $h = 0.01$ построим облако траекторий, вычислим матрицу ковариации и изобразим эмпирическую зависимость $\overline{\lambda_1}(a), \overline{\lambda_2}(a)$ (Рис. \ref{fig:eigenvalues_empirical_FHN}).
...
Вблизи них система ведет себя одинаково с точностью до симметрии. Рассмотрим одно из них, например $(\overline{x}_2, \overline{y}_2)$. На основе симуляции длительностью $T = 100$ с шагом по времени $h = 0.01$ построим облако траекторий, вычислим матрицу ковариации и изобразим эмпирическую зависимость $\overline{\lambda_1}(a), \overline{\lambda_2}(a)$ (Рис. \ref{fig:eigenvalues_empirical_SN}).
...
\newpage
\subsection{Анализ с помощью метода стохастической чувствительности}
...
\begin{cases}
    ...
    \begin{aligned}
        -c \cdot \frac{s_1 + 2b w_2}{2a} + (a + d) w_2 & \\
        - b \cdot \frac{s_2 + 2c w_2}{2d} = 0
    \end{aligned}
\end{cases}
$$
...
Получено явное решение в общем виде, но для удобства программного решения можно представить полученную систему уравнений в матричной форме:
...
Найдем матрицу стохастической чувствительности $W$ для FHN, где
...
Матрицу $W$ можно выразить из системы уравнений Ляпунова:
...
\implies
\begin{cases}
    2\frac{1}{\delta} (1 - x^2) w_1 - 2 \frac{1}{\delta} w_2 = -1 \\
    1 \cdot w_1 + (\frac{1}{\delta} (1 - x^2) + 0) w_2 - \frac{1}{\delta} w_3 = 0 \\
    2 \cdot 1 w_2 + 2 \cdot 0 w_3 = 0
\end{cases}
$$
...
\begin{cases}
    w_2 = 0 \\
    w_1 = -\frac{\delta}{2(1 - x^2)} \\
    w_3 = -\frac{\delta^2}{2(1 - x^2)}
\end{cases}
\implies
W =
\begin{pmatrix}
    -\frac{\delta}{2(1 - x^2)} & 0 \\
    0 & -\frac{\delta^2}{2(1 - x^2)}
\end{pmatrix}
$$

Таким образом, собственные числа матрицы $W$ для FHN имеют вид:

$$
\lambda_1 = -\frac{\delta}{2(1 - x^2)}, \lambda_2 = -\frac{\delta^2}{2(1 - x^2)}.
$$

В равновесии $(x, y) = (-a, \frac{a^3}{3} - a)$ имеем:

$$
\lambda_1 = -\frac{\delta}{2(1 - a^2)}, \lambda_2 = -\frac{\delta^2}{2(1 - a^2)}.
$$

Найдем матрицу стохастической чувствительности $W$ для SN, где
...
Для краткости не будем приводить аналитический вывод матрицы $W$ для модели SN, и численно найдем ее и ее собственные числа с помощью программы для построения графиков собственных чисел.
...
% Найдем собственные числа матрицы $W$ в общем виде:
...
% $$
% (w_1 - \lambda)(w_3 - \lambda) - w_2^2 = 0
% \implies
% \lambda^2 - \lambda (w_1 + w_3) + w_1 w_3 - w_2^2 = 0
% $$

% $$
% \lambda_{1, 2} = \frac{w_1 + w_3 \pm \sqrt{(w_1 + w_3)^2 - 4(w_1 w_3 - w_2^2)}}{2}
% $$

(б) Найдем собственные числа $\lambda_1, \lambda_2$ матрицы стохастической чувствительности $W$.

Построим зависимость $\varepsilon^2 \cdot \lambda_1(a), \varepsilon^2 \cdot \lambda_2(a)$ для значения $\varepsilon$ из пункта 1 (б) для системы FHN (Рис. \ref{fig:eigenvalues_theoretical_FHN}).
...
Построим зависимость $\varepsilon^2 \cdot \lambda_1(a), \varepsilon^2 \cdot \lambda_2(a)$ для $\varepsilon$ из пункта 1 (б) для системы SN (Рис. \ref{fig:eigenvalues_theoretical_SN}).
...
На Рис. \ref{fig:eigenvalues_theoretical_FHN}, \ref{fig:eigenvalues_theoretical_SN} различные значения $N$ не несут математического смысла (графики одинаковы для любых значений $N$, т.к. не зависят от конкретной выборки, а являются теоретическими).
Графики построены таким образом для упрощения программного кода для корректного построения графиков при сравнении эмпирических и теоретических зависимостей в дальнейшем, на Рис. \ref{fig:eigenvalues_FHN}, \ref{fig:eigenvalues_SN}.
...
Эту проблему не удалось устранить с помощью 10-кратного увеличения временного интервала для симуляций (в предыдущей версии лабораторной работы использовалось $T = 10$, а в текущей -- $T = 100$).
Наращивание размера выборки увеличением количества симуляций или уменьшением шага по времени не представляется возможным в силу ограниченных вычислительных ресурсов.
...
\newpage
\subsection{Доверительные эллипсы}
